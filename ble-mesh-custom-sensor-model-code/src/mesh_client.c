...
static bool parse_received_data(esp_ble_mesh_model_cb_param_t *recv_param, model_sensor_data_t *parsed_data)
{
    ...
    if (parsed_data->csum == calculate_checksum(&received_dht11_message))
    {
        ...
        sprintf(data, "{\"Msg ID \":%d}", parsed_data->msg_id);
        esp_mqtt_client_publish(client, TOPIC, data, 0, 1, 0);

        ESP_LOGW("PARSED_DATA", "DEVICE NAME = %s", parsed_data->device_name);
        sprintf(data, "{\"Node name \":%s}", parsed_data->device_name);
        esp_mqtt_client_publish(client, TOPIC, data, 0, 1, 0);
    ...
    else
    {
        ...
        ESP_LOGE("PARSED_DATA", "MSG ID    = %d", parsed_data->msg_id);
        ESP_LOGE("PARSED_DATA", "ttl recv = %d", (model_sensor_data_t *)recv_param->client_recv_publish_msg.ctx->recv_ttl);
        ESP_LOGE("PARSED_DATA", "Device Name = %s", parsed_data->device_name);
        ESP_LOGE("PARSED_DATA", "Temperature = %f", parsed_data->temperature);
        ESP_LOGE("PARSED_DATA", "Humidity    = %f", parsed_data->humidity);
        ESP_LOGE("PARSED_DATA", "Index    = %d", parsed_data->index);
        ESP_LOGE("PARSED_DATA", "Status    = %d", parsed_data->status);
        ESP_LOGE("PARSED_DATA", "CSUM    = %lu", parsed_data->csum);
        return 1;
    }
    return 0;
}
