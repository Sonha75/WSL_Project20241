...
static void parse_received_data(esp_ble_mesh_model_cb_param_t *recv_param, model_sensor_data_t *parsed_data) {
    ...
    ESP_LOGW("PARSED_DATA", "temp_ADC   %d     = %f",parsed_data->index, parsed_data->temp_ADC);
     ...
     ESP_LOGW("PARSED_DATA", "Light   %d     = %lld",parsed_data->index, parsed_data->light);
     sprintf(data,"{\"Light %d\":%lld}",parsed_data->index,parsed_data->light);
     esp_mqtt_client_publish(client, TOPIC , data, 0, 1, 0);

     ESP_LOGW("PARSED_DATA", "smoke   %d     = %lf",parsed_data->index, parsed_data->smoke);
     sprintf(data,"{\"smoke %d\":%lf}",parsed_data->index,parsed_data->smoke);
     esp_mqtt_client_publish(client, TOPIC , data, 0, 1, 0);
