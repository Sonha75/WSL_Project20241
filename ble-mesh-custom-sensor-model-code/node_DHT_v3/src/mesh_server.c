...
static esp_ble_mesh_model_op_t custom_sensor_op[] = {
    ...
    //ESP_BLE_MESH_MODEL_OP(ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATE, 0),
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT:
        ...
        case ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_GET:
            if (param->model_operation.length > 0)
            {
                ...
                _ctx.send_ttl = 7;
                //_ctx.addr = 0xC005;             //multicast
                _ctx.addr = 0xFFFF; // unicast
                //_ctx.recv_dst = 0xC003;         // me
                // _ctx.srv_send = 0;
                _ctx.send_rel = 0;
                esp_err_t err = esp_ble_mesh_server_model_send_msg(custom_models, &_ctx, ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATE, sizeof(dev_state), dev_state);
                if (err != ESP_OK)
                {
                    ESP_LOGE(TAG, "Send STATUS failed err 0x%04x", err);
                }else{
                    //ESP_LOGE(TAG, "Send STATE success err 0x%04x", err);
                }
